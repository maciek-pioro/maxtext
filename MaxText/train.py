...
def record_scalar_metrics(metrics, step_time_delta, per_device_tflops, lr):
    ...
    metrics["scalar"].update(
        {
            "perf/per_device_tflops_per_sec": per_device_tflops
    ...
    if _buffered_metrics is not None:
        ...
        if config.metrics_file:
            max_utils.write_metrics_locally(
                _buffered_metrics, _buffered_step, config, local_metrics_file
            )

        if config.gcs_metrics and jax.process_index() == 0:
            running_gcs_metrics = max_utils.write_metrics_for_gcs(
                _buffered_metrics, _buffered_step, config, running_gcs_metrics
            )
...
def save_checkpoint(
    checkpoint_manager, step, state, dataset_type="c4", data_iterator=None
):
...
def train_step(model, config, state, data, dropout_rng):
    ...
    train_loss_fn = functools.partial(
        loss_fn, model, config, data, dropout_rng, is_train=True
    )
...
def create_goodput_recorder(config):
    if config.enable_goodput_recording:
        ...
        recorder = goodput.GoodputRecorder(
            config.run_name, logger_name, jax.process_index() == 0
        )
        return recorder
    return None
...
def check_example_batch(config, example_batch):
    if config.max_checkify:
        jittable_f = checkify.checkify(
            lambda x: checkify.check(
                jnp.any(x > -1), "Batch contains bad synthetic data!"
            )
        )
        # Check if inputs in batch contains bad synthetic data.
        err, _ = jax.jit(jittable_f)(
            example_batch["inputs"][: config.global_batch_size_to_train_on, :]
        )
        err.throw()
...
def init_logging(config):
    if jax.process_index() == 0:
        ...
        run = neptune.init_run(
            ...
            with_id=config.experiment_id,  # can be None, this starts a new run
        )
...
def setup_train_loop(config):
    ...
    maxtext_utils.assert_params_sufficiently_sharded(
        state.params, mesh, tolerance=params_sharded_tolerance
    )
...
def train_loop(config, state=None):
    ...
    local_metrics_file = (
        open(config.metrics_file, "a", encoding="utf8") if config.metrics_file else None
    )
    ...
    if config.profiler != "" and first_profiling_step >= config.steps:
        raise ValueError(
            "Profiling requested but initial profiling step set past training final step"
        )
    ...
    for step in np.arange(start_step, config.steps):
        ...
        if checkpoint_manager is not None:
            if save_checkpoint(
                checkpoint_manager, int(step), state, config.dataset_type, data_iterator
            ):
        ...
        if (
            ...
            print('eval_data_iterator', eval_data_iterator)
            assert eval_data_iterator
            cumulative_eval_metrics = {"total_loss": 0.0, "total_weights": 0.0}
            eval_batch_count = 0
            for eval_batch in eval_data_iterator:
                print('new_eval_batch')
                if (
                    config.eval_batch_num > 0
                    and eval_batch_count >= config.eval_batch_num
                ):
                    break
                with mesh, nn_partitioning.axis_rules(config.logical_axis_rules):
                    eval_metrics = p_eval_step(state, eval_batch, nextrng)
                cumulative_eval_metrics["total_loss"] += float(
                    eval_metrics["scalar"]["evaluation/total_loss"]
                )
                cumulative_eval_metrics["total_weights"] += float(
                    eval_metrics["scalar"]["evaluation/total_weights"]
                )
                eval_batch_count += 1
            eval_loss = cumulative_eval_metrics["total_loss"] / (
                cumulative_eval_metrics["total_weights"] + EPS
            )
            max_logging.log(
                f"average loss after {step=}: {eval_loss=}, total_weights={cumulative_eval_metrics['total_weights']}"
            )
            if eval_loss <= config.target_eval_loss:
                max_logging.log(
                    f"Early stop and exit loop after reaching {config.target_eval_loss=}"
                )
                prof.deactivate()
                break
...
if __name__ == "__main__":
    ...
    # run_via_mrunner = ((get_configuration is not None) and (not os.environ.get("RUN_WITHOUT_MRUNNER") is not None))
    skip_mrunner = os.environ.get("RUN_WITHOUT_MRUNNER") or get_configuration is None

    if skip_mrunner:
        print("mrunner not found, using app.run instead")
        app.run(main)
    else:
        mrunner_config = get_configuration()

        yaml_config = mrunner_config.from_config

        with open(yaml_config, "r") as f:
            config_dict = yaml.safe_load(f)

        print("config_dict", config_dict)
        print("mrunner_config", mrunner_config)

        mixed_config = {**config_dict, **dict(mrunner_config)}

        if "yaml" in yaml_config:
            yaml_config = yaml_config.replace(".yaml", ".yml")

        new_config_file = yaml_config.replace(".yml", f"_{str(uuid.uuid4())}_mixed.yml")

        with open(new_config_file, "w") as f:
            yaml.dump(mixed_config, f)

        print(f"Munch config: {repr(mrunner_config)}")
        argv = ["train.py", yaml_config]

        app.run(main, argv=["train.py", new_config_file])
